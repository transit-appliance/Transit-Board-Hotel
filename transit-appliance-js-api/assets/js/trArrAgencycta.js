...
function trArrAgencyctaMakeServiceRequests(stops,options,stop_cache,service_requests) {
    
    if (service_requests.cta == undefined) {
	service_requests.cta = [];
    }
    
    for (var stop_id in stops) {
	var stop_data = stop_cache.stopData('cta',stop_id);
